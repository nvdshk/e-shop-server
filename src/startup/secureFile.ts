...
const SECRT_SALT = process.env.SECRET_SALT || 'Secret Salt You Should Update'
const SECRT_PASSPHRASE =
  process.env.SECRET_PASSPHRASE || 'Secret Key You Should Update'

const secretKey = crypto.scryptSync(SECRT_PASSPHRASE, SECRT_SALT, 32) // salt can be random, but in this case we are just using a string

async function encryptFile(inputPath: any, key = secretKey) {
...
async function decryptToString(inputPath: any, key = secretKey) {
...
export default { encryptFile, decryptToString }
